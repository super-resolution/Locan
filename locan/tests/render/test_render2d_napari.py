...
from locan import render_2d_mpl, render_2d_napari, \
    select_by_drawing_napari, render_2d_rgb_napari
...
def test_render_2d_napari_single(make_napari_viewer, locdata_single_localization, caplog):
    ...
    render_2d_napari(locdata_single_localization, viewer=viewer,
                     bin_size=100, cmap='viridis', gamma=0.1)
    viewer.close()
    assert caplog.record_tuples[0] == ('locan.render.render2d', 30, 'Locdata carries a single localization.')
...
def test_select_by_drawing_napari(make_napari_viewer, locdata_blobs_2d):
    ...
    rois = select_by_drawing_napari(locdata_blobs_2d, viewer=viewer, napari_run=False,
                                    bin_size=10, cmap='viridis', gamma=0.1)
    assert len(rois) == 1
    assert repr(rois[0].region) == 'Rectangle((122.0, 565.0), 90.0, 100.0, 0)'
    viewer.close()
