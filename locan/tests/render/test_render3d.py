...
from locan import render_3d, render_3d_napari, scatter_3d_mpl, render_3d_rgb_napari
...
def test_render_3d_napari_coordinates(locdata_blobs_3d):
    viewer, histogram = render_3d_napari(locdata_blobs_3d, bin_size=10, cmap='viridis', gamma=0.1)
    viewer.add_points((locdata_blobs_3d.coordinates - np.array(histogram.bins.bin_range)[:, 0]) / 10 )
    napari.run()
...
def test_scatter_3d_mpl_single(locdata_single_localization, caplog):
    ...
    assert caplog.record_tuples == [('locan.render.render3d', 30, 'Locdata carries a single localization.')]
    # plt.show()
