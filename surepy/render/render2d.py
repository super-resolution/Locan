...
def render2D(locdata, ax, bin_size=10, range='auto', rescale=(2, 98)):
    ...
    Prepare matplotlib axes with the image.
    ...
    range : [[min,max],[min,max]] or 'auto' or 'zero'
        defining the binned region by [min, max] ranges from
        input; for 'auto' by [min, max] ranges from data; for 'zero' by [0, max] ranges from data.
    rescale : tuple or 'equal'
        rescale intensity values to be within percentile or equalize histogram ('equal').
    ...
    if isinstance(range, str):
        if range == 'auto':
            ...
            range = np.array([np.array([0, 0]), [locdata.properties[x] for x in ['Position_x_max', 'Position_y_max']]]).T
    else:
