...
def test_NNDistances_csr_2d():
    ...
    assert dist.pdf(x=np.array([0, 1, 2]), density=1) == pytest.approx([0, 2.71521056e-01, 4.38232365e-05])


def test_NNDistances_csr_3d():
    dist = NNDistances_csr_3d()
    assert dist.shapes == 'density'
    assert dist.pdf(x=np.array([0, 1, 2]), density=1) == pytest.approx([0, 1.90564233e-01, 1.40579528e-13])


def test_Nearest_neighbor_distances_empty(caplog):
    nn_1 = NearestNeighborDistances().compute(LocData())
    nn_1.hist()
    assert caplog.record_tuples == [('locan.analysis.nearest_neighbor', 30, 'Locdata is empty.')]
...
def test_DistributionFits(locdata_simple):
    ...
    ds.fit()
    assert ds.parameter_dict() == {'density': pytest.approx(0.058984374999999166), 'loc': 0, 'scale': 1}
    ds.fit(with_constraints=False)
    assert ds.parameter_dict() == pytest.approx({'density': 0.8900132393760514,
...
def test_DistributionFits_k2(locdata_simple):
    ...
    assert nn_1.distribution_statistics.parameter_dict() == \
           {'density': pytest.approx(0.028906249999999137), 'loc': 0, 'scale': 1}
...
def test_NearestNeighborDistances_3d(locdata_3d):
    ...
    ds = _DistributionFits(nn_1)
    ds.fit()
    assert ds.parameter_dict() == {'density': pytest.approx(0.007666015624999118), 'loc': 0, 'scale': 1}
    ds.fit(with_constraints=False)
    assert ds.parameter_dict() == pytest.approx({'density': 1.0047585928889682,
...
def test_NearestNeighborDistances_1d(caplog, locdata_1d):
    ...
    ds = _DistributionFits(nn_1)
    ds.fit()
    assert caplog.record_tuples == [('locan.analysis.nearest_neighbor', 30, 'No fit model for 1 dimensions available.')]
