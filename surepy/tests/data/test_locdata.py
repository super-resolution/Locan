...
def test_LocData(df_simple):
    dat = LocData(dataframe=df_simple, meta={'comment': 'some user comment'})
    assert (len(dat) == 5)
...
def test_LocData_from_selection(df_simple):
    dat = LocData.from_dataframe(dataframe=df_simple, meta={'comment': 'some user comment'})
    sel = LocData.from_selection(locdata=dat, indices=[1,3,4], meta={'comment': 'some new user comment'})
    assert (len(sel) == 3)
...
def test_LocData_concat(df_simple):
    ...
    sel_1 = LocData.from_selection(locdata=dat, indices=[0, 1, 2])
    sel_2 = LocData.from_selection(locdata=dat, indices=[3, 4])
    col = LocData.concat(sel_1, sel_2, meta={'comment': 'some user comment'})
    assert (len(col) == 5)
    assert (col.meta['comment'] == 'some user comment')

def test_LocData_reduce(df_simple):
    dat = LocData.from_dataframe(dataframe=df_simple, meta={'comment': 'some user comment'})
    sel_1 = LocData.from_selection(locdata=dat, indices=[1,3,4], meta={'comment': 'some new user comment'})
    sel_2 = LocData.from_selection(locdata=dat, indices=[1,3,4], meta={'comment': 'some new user comment'})
    flag = sel_1.reduce()
    assert (flag == 1)
    assert (len(sel_1) == 3)
