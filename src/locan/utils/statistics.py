...
class WeightedMeanVariance(NamedTuple):
    weighted_mean: float | npt.NDArray[np.float64]
    weighted_mean_variance: float | npt.NDArray[np.float64]
...
def biased_variance(
    ...
    return_value: npt.NDArray[np.float64] = variance * (1 - 1 / n_samples)
    return return_value
